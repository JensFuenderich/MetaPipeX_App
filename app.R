...
devtools::install_github("JensFuenderich/MetaPipeX", subdir = "R-Package", force = TRUE)
library(MetaPipeX)

install.packages("remotes")
remotes::install_github("RobbievanAert/puniform")

### general imports
MA_data <- readr::read_csv(here::here("Data","Input","MetaPipeXApp Data Empty.csv"))[,-1]
#IPD_MA_data / MR_data <- readr::read_csv(here::here("Data","Input",""))[,-1]
codebook <- readr::read_csv(here::here("Data","Input","MetaPipeXApp codebook.csv"))
codebook_text_vec <- readr::read_file(here::here("Data","Input","MetaPipeXApp codebook_text.txt"))

### helpers
# create empty object which will be overwritten by working in the "Reactive Data Table" tab
Reactive_Output_Data <- c()

saveRDS(as.data.frame(matrix(NA, ncol = 1, nrow = 1)), "Data/Input/Data_Exclusions.rds")
saveRDS(as.data.frame(matrix(NA, ncol = 2, nrow = 2)), file = "Data/Output/Plot Data/hist_plot_data.rds")
saveRDS(as.data.frame(matrix(NA, ncol = 2, nrow = 2)), file = "Data/Output/Plot Data/violin_plot_data.rds")
saveRDS(as.data.frame(matrix(NA, ncol = 2, nrow = 2)), file = "Data/Output/Plot Data/scatter_plot_data.rds")
saveRDS(as.data.frame(matrix(NA, ncol = 2, nrow = 2)), file = "Data/Output/Plot Data/funnel_plot_data.rds")
...
server <- function(input, output, session){
  ...
  IPD_raw_data_input_columns <- shiny::reactive({
    ...
    if (length(input$IPD) > 0) {
      ...
      # store input
      save(IPD_list,
           file = here::here("Data", "Input", "UI Upload", "IPD_raw_upload.Rda"))
  ...
  IPD_data_input <- eventReactive( input$confirm_upload, {

    # IPD_list <- readRDS(here::here("Data", "Input", "UI Upload", "IPD_raw_upload.Rds"))

    load(here::here("Data", "Input", "UI Upload", "IPD_raw_upload.Rda"))
    ...
    withProgress(message = 'Calculation in progress. This may take a moment.',
                 ...
                 style = "old",
                 {
                   ...
                   reduce_cols <- function(x){
                     ...
                     IPD_list[[x]] <- subset(IPD_list[[x]], select =  c(if(input$create_custom_multilab_col == TRUE){"MultiLab"}else{input$multilab_col},
                   ...
                   # remove NA
                   IPD_list <- lapply(1:length(IPD_list), function(x){IPD_list[[x]] <- na.omit(IPD_list[[x]])})
  ...
  ReplicationSum_data_input <- eventReactive( input$confirm_upload, {
    ...
    # store input
    save(ReplicationSum_list,
         file = here::here("Data", "Input", "UI Upload", "ReplicationSum_raw_upload.Rda"))
    ...
    withProgress(message = 'Calculation in progress. This may take a moment.',
                 ...
                 style = "old",
                 {
                   ...
                   ReplicationSum_merged <- MetaPipeX::merge_replication_summaries(data = ReplicationSum_list,
                                                                                   output_folder = "Data/Output/MetaPipeX_Output/Merged Replication Summaries/")

                   # run meta analyses
                   ReplicationSum_analyzed <- MetaPipeX::meta_analyses(data = ReplicationSum_merged$merged_replication_summaries,
                                                                       output_folder = "Data/Output/MetaPipeX_Output/Meta Analyses/")
                   ...
                   names(MetaPipeX_Data)[first_replication_col:last_replication_col] <- stringr::str_c("Replication__Result__", names(MetaPipeX_Data[,first_replication_col:last_replication_col]))

                   # MA
                   first_replication_MA <- last_replication_col + 1
                   last_replication_MA <- ncol(MetaPipeX_Data)
                   names(MetaPipeX_Data)[first_replication_MA:last_replication_MA] <- stringr::str_c("MA__", names(MetaPipeX_Data[,first_replication_MA:last_replication_MA]))
                   ...
                   abbr_library <- as.data.frame(base::rbind(c("_T_", "__treatment group_"),
                                                             ...
                                                             c("__SE_Tau2_", "__standard error of_´Tau2 for_"),
                                                             c("__Tau_", "__Tau for_"),
                   ...
                   description_vector <- stringr::str_replace_all(description_vector, "__Result__", "_")
                   ...
                   # export codebook
                   utils::write.csv(codebook_for_meta_pipe_x,
                                    file = "Data/Output/MetaPipeX_Output/Meta Pipe X/codebook_for_meta_pipe_x_data.csv",
                                    row.names = FALSE)
                   # export data
                   utils::write.csv(MetaPipeX_Data,
                                    file = "Data/Output/MetaPipeX_Output/Meta Pipe X/MetaPipeX_Data.csv",
                                    row.names = FALSE)
                 })
  ...
  MergedReplicationSum_data_input <- eventReactive( input$confirm_upload, {
    ...
    # store input
    save(MergedReplicationSum,
         file = here::here("Data", "Input", "UI Upload", "MergedReplicationSum_raw_upload.Rda"))
  ...
  data <- reactive({
    ...
    # this line of code exists purely, to create a dependency between the reactive object data() and the remove exclusions process
    # it does not and should not produce any output
    ...
    #     df <- subset(df, abs(df$MA__Est__SMD) > 0.1 )
    # }
    ...
    # export data
    write.csv(df,
              file = "Data/Output/Reactive Output.csv",
              row.names = FALSE)
    df
  ...
  data_to_be_excluded <- reactive({
    ...
    if (ncol(readRDS("Data/Input/Data_Exclusions.rds")) > 1) {
      existing_exclusions <- dplyr::left_join(readRDS("Data/Input/Data_Exclusions.rds"), data())
    } else {
      existing_exclusions <- readRDS("Data/Input/Data_Exclusions.rds")
    }
  ...
  observeEvent(input$exclusion,{
    saveRDS(as.data.frame(data_to_be_excluded()), "Data/Input/Data_Exclusions.rds")
  })

  data_excluded <- eventReactive(input$exclusion | input$remove_exclusion,{
    readRDS("Data/Input/Data_Exclusions.rds")
  })
  ...
  observeEvent(input$remove_exclusion, {
    ...
    saveRDS(as.data.frame(data_excluded_removed()), "Data/Input/Data_Exclusions.rds")
  ...
  remaining_data <- eventReactive(input$remove_exclusion | input$exclusion,{
    ...
    if (ncol(readRDS("Data/Input/Data_Exclusions.rds")) > 1 ) {
      ...
      exclusions_full_columns <- dplyr::left_join(readRDS("Data/Input/Data_Exclusions.rds"), data)
      ...
      saveRDS(dplyr::setdiff(data, exclusions_full_columns), "Data/Input/Final_Data.rds")

      dplyr::setdiff(data, exclusions_full_columns)

    } else {

      saveRDS(as.data.frame(data()), "Data/Input/Final_Data.rds")
      data()
  ...
  hist_plot <- shiny::reactive({
    ...
    saveRDS(hist_data, "Data/Output/Plot Data/hist_plot_data.rds")
  ...
  hist_data_table_reactive <- reactive({
    ...
    hist_data <- readRDS(file = "Data/Output/Plot Data/hist_plot_data.rds")
    ...
    if (is.null(input$hist_hover) == FALSE) {
      ...
      if (input$hist_include_variable2 == TRUE & input$hist_include_variable3 == TRUE) {
        subset(data,
               (data[,glue::glue("{input$hist_data1}")] < (round(input$hist_hover$x, 0)  +  max(hist_data$Data, na.rm = TRUE)/80) &
                  data[,glue::glue("{input$hist_data1}")] > (round(input$hist_hover$x, 0)  -  max(hist_data$Data, na.rm = TRUE)/80) ) |
                 (data[,glue::glue("{input$hist_data2}")] < (round(input$hist_hover$x, 0)  +  max(hist_data$Data, na.rm = TRUE)/80) &
                    data[,glue::glue("{input$hist_data2}")] > (round(input$hist_hover$x, 0)  -  max(hist_data$Data, na.rm = TRUE)/80) ) |
                 (data[,glue::glue("{input$hist_data3}")] < (round(input$hist_hover$x, 0)  +  max(hist_data$Data, na.rm = TRUE)/80) &
                    data[,glue::glue("{input$hist_data3}")] > (round(input$hist_hover$x, 0)  -  max(hist_data$Data, na.rm = TRUE)/80) ) )
      } else if (input$hist_include_variable2 == TRUE & input$hist_include_variable3 != TRUE){
        subset(data,
               (data[,glue::glue("{input$hist_data1}")] < (round(input$hist_hover$x, 0)  +  max(hist_data$Data, na.rm = TRUE)/80) &
  ...
  ## create data table
  output$hist_data_table <-  DT::renderDataTable({
  ...
  violin_plot_data <- shiny::reactive({
    ...
    violin_data$Statistic <- str_replace(violin_data$Statistic, "CoeffVar for ", "")
    ...
    saveRDS(violin_data, file = "Data/Output/Plot Data/violin_plot_data.rds")
  ...
  output$violin_plot <- renderPlot({
    ...
    if (input$violin_include_point_size == TRUE) {
      ...
      violin_plot_output <- p + geom_boxplot(width = 0.1) +
        ...
        scale_size_continuous( str_replace(str_replace(subset(codebook, codebook$Variable_Name == input$violin_point_size)$Variable_Description, "meta analysis level: ", ""), "lab level: ", "") )
  ...
  violin_data_table_reactive <- reactive({
    ...
    violin_data <- readRDS(file = "Data/Output/Plot Data/violin_plot_data.rds")
  ...
  ## create data table
  output$violin_data_table <-  DT::renderDataTable({
  ...
  scatter_plot_data <- reactive({
    ...
    plot_data$cor_x_y <- rep(cor(na.omit(data.frame( X = plot_data$X, Y = plot_data$Y)))[1,2], nrow(plot_data))

    saveRDS(plot_data, file = "Data/Output/Plot Data/scatter_plot_data.rds")
  ...
  # vals_scatter <- reactiveValues(
  #   keeprows = rep(TRUE, nrow(readRDS("Data/Output/Plot Data/scatter_plot_data.rds"))),
  #   keeprows1 = rep(TRUE, nrow(readRDS("Data/Output/Plot Data/scatter_plot_data.rds")))
  # )
  ...
  output$scatter_plot <- renderPlot({
    ...
    scatter_plot_output <- ggplot(plot_data, aes(x = X, y = Y)) +
      geom_point(aes(colour = if (input$include_point_color == TRUE) {Point_Color}else{},
                     size = if (input$include_point_size == TRUE) {Point_Size}else{} )) +
      theme_light() +
      ggtitle(glue::glue("Correlation between currently depicted X and Y data: {round(unique(plot_data$cor_x_y), digits = 2)} (NA removed)")) +
      xlab(subset(codebook, codebook$Variable_Name == input$x_plot)$Variable_Description) +
      ylab(subset(codebook, codebook$Variable_Name == input$y_plot)$Variable_Description) +
      scale_colour_continuous( if (input$include_point_color == TRUE) {subset(codebook, codebook$Variable_Name == input$color_plot)$Variable_Description}else{} ) +
      scale_size_continuous( if (input$include_point_size == TRUE) {subset(codebook, codebook$Variable_Name == input$size_plot)$Variable_Description}else{} )
  ...
  scatter_data_table_reactive <- reactive({
    ...
    scatter_data <- readRDS(file = "Data/Output/Plot Data/scatter_plot_data.rds")
    ...
    if (is.null(input$scatter_hover) == FALSE) {
      ...
      subset(data, data[,glue::glue("{input$x_plot}")] < (round(as.numeric(input$scatter_hover[1]), 0) +  max(scatter_data$X, na.rm = TRUE)/80) &
               data[,glue::glue("{input$x_plot}")] > (round(as.numeric(input$scatter_hover[1]), 0) -  max(scatter_data$X, na.rm = TRUE)/80) &
               data[,glue::glue("{input$y_plot}")] < (round(as.numeric(input$scatter_hover[2]), 0) +  max(scatter_data$Y, na.rm = TRUE)/80) &
               data[,glue::glue("{input$y_plot}")] > (round(as.numeric(input$scatter_hover[2]), 0) -  max(scatter_data$Y, na.rm = TRUE)/80)
  ...
  ## create data table
  output$scatter_data_table <-  DT::renderDataTable({
  ...
  funnel_plot <- reactive({
    ...
    saveRDS(plot_data, file = "Data/Output/Plot Data/funnel_plot_data.rds")
  ...
  funnel_data_table_reactive <- reactive({
    ...
    funnel_data <- readRDS(file = "Data/Output/Plot Data/funnel_plot_data.rds")
    ...
    if (is.null(input$funnel_hover) == FALSE) {
      ...
      subset(data, data[,glue::glue("{input$funnel_data_est}")] < (round(as.numeric(input$funnel_hover[1]), 2) +  max(funnel_data$Est, na.rm = TRUE)/20) &
               data[,glue::glue("{input$funnel_data_est}")] > (round(as.numeric(input$funnel_hover[1]), 2) -  max(funnel_data$Est, na.rm = TRUE)/20) &
               data[,glue::glue("{input$funnel_data_SE}")] < (round(as.numeric(input$funnel_hover[2]), 2) +  max(funnel_data$SE, na.rm = TRUE)/20) &
               data[,glue::glue("{input$funnel_data_SE}")] > (round(as.numeric(input$funnel_hover[2]), 2) -  max(funnel_data$SE, na.rm = TRUE)/20))
    }else{}
  ...
  ## create data table
  output$funnel_data_table <-  DT::renderDataTable({
